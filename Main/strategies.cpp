...
void avoidEdge() 
{
  ...
  if (anyEdge(&boardSide)) {                                              // Checa se vê alguma borda
    ...
    if(boardSide == 0)                                                    // Caso esteja vendo borda nos dois sensores..
    {
      ...
      move(lastToSee, maxPWM*0.4); // Deveria ter reverse, ver isso      // depois, vira para o centro da arena, sempre checando se está vendo
    ...
    else                                                                  // Caso esteja vendo borda em somente um sensor..
    {
      ...
      move(lastToSee, maxPWM*0.4);                                       // depois, vira para o centro da arena, sempre checando se está vendo
      myDelay(delaySpinEdge);
    }
  }
}
