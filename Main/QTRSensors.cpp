...
void QTRSensors::emittersOff()
{
    ...
    digitalWriteFast(_emitterPin, LOW);
...
void QTRSensors::emittersOn()
{
    if (_emitterPin == QTR_NO_EMITTER_PIN)
        return;
    pinMode(_emitterPin, OUTPUT);
    digitalWriteFast(_emitterPin, HIGH);
    delayMicroseconds(200);
}
...
void QTRSensorsRC::readPrivate(unsigned int *sensor_values)
{
    ...
    for(i = 0; i < _numSensors; i++)
    {
        ...
        digitalWriteFast(_pins[i], HIGH);      // drive sensor line high
    }
    ...
    for(i = 0; i < _numSensors; i++)
    {
        ...
        digitalWriteFast(_pins[i], LOW);        // important: disable internal pull-up!
    }
    ...
    while (micros() - startTime < _maxValue)
    {
        ...
        for (i = 0; i < _numSensors; i++)
        {
            if (digitalReadFast(_pins[i]) == LOW && time < sensor_values[i])
